...
def bench_gromacs(top, gro, steps):
    ...
    if cached.get(grompp_command) is not None:
        ...
        logger.info(
            f"Cached gromacs time grompp: {grompp:.2f} s; mdrun: {mdrun:.2f} s available for {grompp_command}"
        )
        return grompp, mdrun
...
def finish_daemon(result_dir, name, reactive_runs, non_reactive_runs):
    ...
    reactive_data = []
    non_reactive_data = []
    reactive_parsetime = 0.0
    reactive_runtime = 0.0
    non_reactive_parsetime = 0.0
    non_reactive_runtime = 0.0
    reactive_probe = 0.0
    non_reactive_probe = 0.0
    for i, (run, parsetime, runtime, probe) in enumerate(reactive_runs):
        ...
        # averages over n
        reactive_parsetime = (
            parsetime if i == 0 else (reactive_parsetime * (i - 1) + parsetime) / i
        )
        reactive_runtime = (
            runtime if i == 0 else (reactive_runtime * (i - 1) + runtime) / i
        )
    ...
    for i, (run, parsetime, runtime, probe) in enumerate(non_reactive_runs):
        ...
        # averages over n
        non_reactive_parsetime = (
            parsetime if i == 0 else (non_reactive_parsetime * (i - 1) + parsetime) / i
        )
        non_reactive_runtime = (
            runtime if i == 0 else (non_reactive_runtime * (i - 1) + runtime) / i
        )
        non_reactive_probe = (
            probe if i == 0 else (non_reactive_probe * (i - 1) + probe) / i
        )

    parse_slowdown = (
        (reactive_parsetime - non_reactive_parsetime) / non_reactive_parsetime * 100.0
    )
    run_slowdown = (
        (reactive_runtime - non_reactive_runtime) / non_reactive_runtime * 100.0
    )
    logger.info(
...
def run_recipe(recipe_name, data):
    ...
    for entry in data:
        ...
        reac_data, non_reac_data, parse_slowdown, run_slowdown, rprobe, nprobe = (

...
class CustomDonorAcceptor(Force):
    ...
    def __init__(self, sysstar):
        super(CustomDonorAcceptor, self).__init__(sysstar)
    ...
    def _set_force_obj(self):
        ...
        self._force_obj = mm.CustomHbondForce(
            # bond double counted since everything is both donor and acceptor
            # angles only half defined per pair, donor bias
            # so not double counted
            "V_bond/2+V_angle+V_dihedral/2;"
            # dihedral
            ...
            "martini=step(R_min-r)*V_martini_min+step(r-R_min)*V_martini;"
        ...
        # parameters - martini
        self._force_obj.addGlobalParameter(
            "sigma", 0.41
        )
        self._force_obj.addGlobalParameter(
            "epsilon", 2.6
    ...
    def _add_to_force_obj(self, params):
        i, j, k = params
        d = self._force_obj.addDonor(i, j, k, [])

...
def read_atoms(path):
    ...
    for frame in range(n_frames):
        ...
        names[frame] = np.frombuffer(data[start : start + (8 * n_atoms)], dtype="a8")
...
def read_bonds(path: str, read_n_atoms=True) -> tuple[int, int, list[np.ndarray]]:
    ...
    while frame_start < len(data):
        ...
        frames.append(
            np.frombuffer(data[frame_start + 8 : frame_end], dtype=np.uint32).reshape(
                (n_bonds, 2)
            )
        )
...
def generate_vmd_readable_bonds(
    ...
    with open(out_path, "wb") as handle:
        for i, frame in enumerate(frames):
            ...
            box = np.array(
                [xtc_frame.box[0][0], xtc_frame.box[1][1], xtc_frame.box[2][2]],
                dtype=np.float32,
            )
            n_atoms = len(pos)
            bonds = [[] for _ in range(n_atoms)]
            crosses = np.any(
                np.abs(pos[frame[:, 0]] - pos[frame[:, 1]]) / box > 0.5, axis=-1
            )
                ...
                comp_obj.compress(
                    (
                        ...
                        + "} {".join(
                            [
                                " ".join(
                                    [f"{other_atom}" for other_atom in other_atom_list]
                                )
        ...
        for line in lines:
            if line[:5] == "Frame":
                ...
                if reactions.get(name) is None:
                    reactions[name] = np.zeros(frame_count // interval + 1, np.int64)
...
def parse_frag_counts(path, interval, frame_count):
    ...
    with open(path, "r") as f:
        ...
        for line in lines:
            if line[:5] != "Frame":
                continue
            toks = [tok.strip().split(":") for tok in line.strip().split(",")]
            ...
            for [name, count] in toks[1:]:
                if counts.get(name) is None:
                    counts[name] = np.zeros(frame_count // interval + 1, np.int64)

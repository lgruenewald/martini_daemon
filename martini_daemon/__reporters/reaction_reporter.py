...
class ReactionReporter(Reporter):
    def __init__(self, traj_format: str | None = None) -> None:
        ...
        self.traj_format = (
            traj_format
            if traj_format is None or traj_format[0] == "."
            else f".{traj_format}"
        )
    ...
    def on_reaction(
        ...
        if self.traj_format is not None:
            ...
            self.traj_writer.write_frame(
                simulation.current_step, simulation.time_ps, box, pos
            )
    ...
    @classmethod
    def read_reactions(cls, path: str) -> list[ReactionsFileReaction]:
        """

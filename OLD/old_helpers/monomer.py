...
def read_monomer_graph(bond_frames: list[np.ndarray], mapping: np.ndarray):
    """
...
def conversion(
    monomer_frames: list[np.ndarray], start: int, end: int,
...
def bond_orders(
    monomer_frames: list[np.ndarray], start: int, end: int,
    max_bonds_per_monomer: int
) -> np.ndarray:
    """
        Calculate the population for each bond order (how many other monomers
        are bonded to a monomer) within the range start to end.
    """

    count = end - start
    populations = np.zeros((len(monomer_frames), max_bonds_per_monomer+1), np.int64)
    for i, frame in enumerate(monomer_frames):
        orders = np.zeros((count,), dtype=np.int64)
        for bond in frame:
            orders[bond[0] - start] += 1
            orders[bond[1] - start] += 1
        for order in orders:
            if order <= max_bonds_per_monomer:
                populations[i, order] += 1
            else:
                raise ValueError(
                    f"Order {order} reached, increase max bonds per monomer!"
                )
    return populations


def _unroll(max: int, edge_array: np.ndarray):
    """

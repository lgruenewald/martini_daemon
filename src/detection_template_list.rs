...
impl DetectionTemplateList {
    ...
    pub fn add_detection_template(
        &mut self,
        rx: Py<DetectionTemplate>,
        py: Python,
    ) -> PyResult<bool> {
        ...
        if rx_ref.reactants.len() > 3 {
            return Err(PyException::new_err(
                "Only up to 3 reactants per reaction are supported.",
            ));
        }
        ...
        for (i1, a1, i2, a2, r_max) in rx_ref.distance_max.iter() {
            ...
            self.largest_r_max = f64::max(self.largest_r_max, *r_max);
            ...
            if !self.frag_name_to_r_max_atoms.contains_key(frag_name1) {
                self.frag_name_to_r_max_atoms
                    .insert(frag_name1.to_string(), Vec::new());
            }
            self.frag_name_to_r_max_atoms
                .get_mut(frag_name1)
                .unwrap()
            ...
            if !self.frag_name_to_r_max_atoms.contains_key(frag_name2) {
                self.frag_name_to_r_max_atoms
                    .insert(frag_name2.to_string(), Vec::new());
            }
            self.frag_name_to_r_max_atoms
                .get_mut(frag_name2)
                .unwrap()
        ...
        if rx_ref.reactants.len() > 1 {
            if !self.first_reactants.contains_key(&rx_ref.reactants[0]) {
                self.first_reactants
                    .insert(rx_ref.reactants[0].clone(), (false, false));
            }
            match (
                rx_ref.reactants.len(),
                self.first_reactants.get_mut(&rx_ref.reactants[0]),
            ) {
        ...
        if let Some(entry) = self.reactions_by_reactants.get_mut(&reactants) {
            ...
            self.reactions_by_reactants
                .insert(reactants, vec![rx_ref.name.clone().unwrap()]);
        }
